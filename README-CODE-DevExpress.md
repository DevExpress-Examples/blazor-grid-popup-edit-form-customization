...
Both buttons call the `ShowPopup()` method to initialize the [edit model](https://docs.devexpress.com/Blazor/404759/components/grid/editing-and-validation/edit-model) and display the popup form. The `ClosePopup()` method clears the edit model and hides the popup (used by the popup form's **Close** button).
